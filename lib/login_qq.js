...
pt.plogin = {
    ...
    knownEmail: (pt.ptui.style == 25 ? ["qq.com", "vip.qq.com", "foxmail.com"] : ["qq.com", "foxmail.com", "gmail.com", "hotmail.com", "yahoo.com", "sina.com", "163.com", "126.com", "vip.qq.com", "vip.sina.com", "sina.cn", "sohu.com", "yahoo.cn", "yahoo.com.cn", "139.com", "wo.com.cn", "189.cn", "live.com", "msn.com", "live.hk", "live.cn", "hotmail.com.cn", "hinet.net", "msa.hinet.net", "cm1.hinet.net", "umail.hinet.net", "xuite.net", "yam.com", "pchome.com.tw", "netvigator.com", "seed.net.tw", "anet.net.tw"]),
    qrlogin_clock: 0,
    qrlogin_timeout: 0,
    ...
    qr_uin: "",
    qr_nick: "",
    onekey_verify_timeout: 60 * 60 * 1000,
    onekeyVerifyClock: 0,
    dftImg: "",
    ...
    aqScanLink: "<a href='javascript:void(0)'; onclick='pt.plogin.switch_qrlogin()'>" + (pt.ptui.lang == "2052" ? "立即扫描" : (pt.ptui.lang == "1028" ? "立即掃描" : "Scan now")) + "</a>",
    isNewQr: false,
    hasNoQlogin: false,
    checkRet: -1,
    cap_cd: 0,
    ...
    isTenpay: pt.ptui.style == 34,
    isMailLogin: pt.ptui.style == 25 || pt.ptui.style == 30,
    switchpageCount: 0,
    domFocus: function (b) {
        try {
        ...
        var c = $.str.utf8ToUincode($.cookie.get("ptnick_" + b)) || b;
    ...
    }, initQlogin: function (d, c) {
        d = d || pt.plogin.initQlogin.url;
        pt.plogin.initQlogin.url = d;
        var g = 0;
        var a = false;
        if (d && pt.ptui.auth_mode == 0) {
            a = true
        }
        if (!c && pt.ptui.enable_qlogin != 0 && $.cookie.get("pt_qlogincode") != 5) {
            g = $.getLoginQQNum()
        }
        g += a ? 1 : 0;
        g += c ? c.length : 0;
        if (g == 0) {
            ...
            pt.qlogin.buildQloginList(c)
        }
        if (pt.ptui.auth_mode != 0 && d) {
            pt.plogin.showAuth(pt.ptui.auth_mode, d)
        }
        if (pt.ptui.enable_qlogin == 0 && $.sso_ver == 0) {
            try {
                if ($.suportActive()) {
                    var b = new ActiveXObject("SSOAxCtrlForPTLogin.SSOForPTLogin2").QuerySSOInfo(1);
                    $.sso_ver = b.GetInt("nSSOVersion")
                } else {
                    if (!$.nptxsso) {
                        $.nptxsso = document.createElement("embed");
                        $.nptxsso.type = "application/nptxsso";
        ...
        case 1:
            ...
            if (pt.plogin.armSafeEdit && pt.plogin.armSafeEdit.everSafe) {
                pt.plogin.armSafeEdit.lockToggle();
                pt.plogin.armSafeEdit.everSafe = false
            }
            if (pt.plogin.onekeyVerifyClock != 0) {
                pt.plogin.onekeyVerify("normal")
            }
            break;
        case 2:
            ...
            if (pt.plogin.armSafeEdit.isSafe) {
                pt.plogin.armSafeEdit.lockToggle();
                pt.plogin.armSafeEdit.everSafe = true
            }
            break
        }
        window.setTimeout(function () {
            try {
                if (pt.plogin.switchpageCount == 1) {
        ...
        if (pt.ptui.lockuin) {
            ...
            pt.plogin.initQlogin(a)
        } if (pt.plogin.isTenpay && $.sso_ver >= 1093 && !(pt.plogin.isWin8() && $.suportActive())) {
            pt.plogin.armSafeEdit()
        }
        window.setTimeout(function () {
            pt.plogin.domLoad()
        }, 100)
        ...
        var a = navigator.userAgent.toLowerCase();
        if (a.indexOf("windows nt 6.2") > -1 || a.indexOf("windows nt 6.3") > -1) {
            return true
        } else {
            return false
        }
        ...
        $("u").readOnly = true;
        var b = $("uinArea");
        if (!$.css.hasClass(b, "default")) {
            $.css.addClass(b, "default")
        }
        var a = $("uin_del");
        ...
        if (a.value == "") {
            var b = $("uin_tips");
            $.css.show(b);
            b.className = "input_tips"
        } else {
            pt.plogin.set_account();
            pt.plogin.check()
        }
        a.parentNode.className = "inputOuter"
        ...
        } if (pt.plogin.isNewQr && pt.plogin.loginState == pt.LoginState.QLogin) {
            if (b) {
                pt.plogin.begin_qrlogin()
            } else {
                window.setTimeout(function () {
        ...
        if (pt.plogin.isTenpay) {
            if ($.sso_ver >= 1093) {
                $.report.monitor("451205")
            } else {
                $.report.monitor("451206")
            }
        }
        ...
        if (pt.ptui.gzipEnable == "1" || pt.ptui.isHttps) {} else {
        ...
        if (domVip2) {
            $.e.add(domVip2, "click", function (e) {
        ...
        if (pt.plogin.isNewQr) {
            $.e.add(document, "visibilitychange", function (e) {
                ...
                case "hidden":
                    ...
                    break;
                case "visible":
                    if (pt.plogin.loginState == pt.LoginState.QLogin && pt.plogin.qrlogin_invalid) {
                        pt.plogin.begin_qrlogin()
                    }
                    break
                }
        ...
        var b = $("newVcodeArea");
        b.style.cssText = "background: none #FFFFFF; position: absolute; top: 0; width: 100%; z-index:9999;";
        if (pt.ptui.style == 25) {
            b.style.width = "370px"
        }
        ...
        try {
            ...
            if ($.str.trim(b.value) == "") {
                ...
                pt.plogin.domFocus(b);
                return false
            }
            if ($.check.isNullQQ(b.value)) {
                pt.plogin.show_err(pt.str.inv_uin);
                pt.plogin.domFocus(b);
                return false
            }
            var f = g.value;
            if (pt.plogin.armSafeEdit.isSafe && a) {
                f = a.GetPwdHash();
                if (f == "D41D8CD98F00B204E9800998ECF8427E") {
                    f = ""
                }
                pt.plogin.armSafeEdit.safepwd = f
            }
            if (!f) {
                pt.plogin.show_err(pt.str.no_pwd);
                pt.plogin.domFocus(g);
                return false
            }
            if (h.value == "") {
        ...
        return a
    }, ptui_uin: function (a) {}, is_mibao: function (a) {
        return /^http(s)?:\/\/ui.ptlogin2.(\S)+\/cgi-bin\/mibao_vry/.test(a)
    }, __get_polling_url: function (b) {
    ...
    }, qr_load: function (a) {}, qr_error: function (a) {}, switch_qrlogin_animate: function () {
        ...
        if ($.browser("version") == "6.0") {
            q.background = q.background.replace("icon_5.png", "icon_5_8.png")
        }
        for (var f in q) {
            k.style[f] = q[f]
        }
        k.style.style = "right";
        pt.plogin.armSafeEdit.isSafe = false;
        n.appendChild(k);
        ...
        var j = '<object id="safe_edit" classid="CLSID:EAAED308-7322-4b9b-965E-171933ADD473" width="' + d + '" height="' + m + '">                    <param name="bkColor" value="16777215"/>                    <param name="fontColor" value="0"/>                    <param name="fontHeight" value="0.25" />                    <param name="caretHeight" value="0.2" />                    <param name="borderType" value="2" />                    <param name="borderColor" value="16777215" />                </object>';
        var a = '<embed id="safe_edit" type="application/nptxsso"     width="' + d + '" height="' + m + '" bkcolor="16777215" fontcolor="0"     fontheight="0.25" caretheight="0.8" bordertype="2" bordercolor="16777215" />';
        var h = '<div class="safe-edit-tips"     style="color: #2C9E62; height: 28px; line-height: 34px; width: ' + d + 'px;">    当前为安全登录模式，使用密码控件</div>';
        $.e.add(k, "click", g);
        pt.plogin.armSafeEdit.lockToggle = g;
        ...
        function g() {
            if (pt.plogin.armSafeEdit.isSafe) {
                ...
                n.removeChild(b);
                k.style.backgroundPosition = "-130px -130px";
                pt.plogin.armSafeEdit.isSafe = false
            } else {
                var o = $("safe_edit");
                if (o) {
                    ...
                    b.innerHTML = ($.suportActive() ? j : a) + h;
                    n.appendChild(b);
                    o = $("safe_edit");
                    $.e.add(o, "focus", pt.plogin.check)
                }
                try {
                    o.CreateSafeEdit();
                    o.ClearAllInput();
                    ...
                    k.style.backgroundPosition = "-117px -130px";
                    pt.plogin.armSafeEdit.isSafe = true
                } catch (p) {
        ...
        c = parseInt(c) || pt.plogin.onekeyVerify.__style || 1;
        pt.plogin.onekeyVerify.__style = c;
        if (d == "hide") {
            ...
            if (d == "invalid") {
                ...
                $.css.setClass(h, "ov-" + c);
                a = a || pt.plogin.onekeyVerify.__tips || "您的帐号千金难求。为确保安全，请务必对手机收到的登录请求进行确认。";
                pt.plogin.onekeyVerify.__tips = a;
                pt.plogin.show_err(a, true);
...
function ptuiCB(l, o, b, k, d, a) {
    var g = $("p");
    var n = pt.plogin.at_account && (g.value || pt.plogin.armSafeEdit.safepwd);
    clearTimeout(pt.plogin.loginClock);
    ...
    case "10006":
        ...
        break;
    case "10008":
        pt.plogin.onekeyVerify("normal", o, d);
        return;
    default:
    ...
    if (Math.random() < 0.2) {
        ...
        var m = "flag1=7808&flag2=7&flag3=2&" + j + "=" + (pt.plogin.isdTime["7808-7-2-1"] - pt.plogin.isdTime["7808-7-2-0"]);
        $.report.simpleIsdSpeed(m)
    }
}
...
function ptui_checkVC(a, d, b, f, c) {
    ...
    case "3":
        ...
        if (!d) {
            $.report.nlog("check no code return,ret=" + a + ",code=" + d + ",uin=" + $.str.bin2String(b))
        }
        break;
    case "1":
        pt.plogin.cap_cd = d;
